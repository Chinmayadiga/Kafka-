...
FILE_PATH = r"\Users\user\Desktop\kafka\restaurant_orders.csv"
...
def get_restaurant_instance(file_path):
    ...
    for data in df.values:
        rest=Restaurant(dict(zip(columns,data)))
        rests.append(rest)
        yield rest

def restaurant_to_dict(rest:Restaurant, ctx):
    """
...
def main(topic):
    ...
    producer.poll(0.0)
    try:
        for rest in get_restaurant_instance(file_path=FILE_PATH):

            print(rest)
            producer.produce(topic=topic,
                            key=string_serializer(str(uuid4()), restaurant_to_dict),
                            value=json_serializer(rest, SerializationContext(topic, MessageField.VALUE)),
